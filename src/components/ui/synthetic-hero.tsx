    ...
    useGSAP(
        () => {
            ...
            document.fonts.ready.then(() => {
                // Fallback for SplitText since it's a premium plugin not always available in free environments
                // We'll just animate the container for now to be safe, or implement a simple splitter if needed.
                // For this implementation, I will assume we might not have the paid SplitText plugin and do a simple fade/slide up
                // If SplitText IS available, we can uncomment. But to ensure it works without errors:

                // Standard GSAP animation without SplitText dependency for safety
        ...
        >
            ...
            <div className="relative z-10 flex flex-col items-center text-center px-6">
                <div ref={badgeWrapperRef}>
                    <Badge className="mb-6 bg-white/10 hover:bg-white/15 text-emerald-300 backdrop-blur-md border border-white/20 uppercase tracking-wider font-medium flex items-center gap-2 px-4 py-1.5 cursor-default">
                ...
                >
                    {ctaButtons.map((button, index) => {
                        ...
                        const classes = isPrimary
                            ? "px-8 py-3 rounded-xl text-base font-medium backdrop-blur-lg bg-emerald-400/80 hover:bg-emerald-300/80 shadow-lg transition-all cursor-pointer border-0"
